...
class VGGSoundDataset(data.Dataset):
    ...
    def map_embeddings_target(self):
        # w2v_embedding = torch.Tensor(self.data["text"]["data"])[sorted(self.classes)].cuda() # orig
        w2v_embedding = torch.Tensor(self.data["text"]["data"])[sorted(self.classes)].to(self.args.device)
    ...
    def features_processed_folder(self):
        ...
        return self.root / "_features_processed"
        ...
        if modality == "text":
            ...
            if self.args.use_wavcaps_embeddings == True:
                data_raw_wavcaps = np.load(
                    (
                            self.root / "features" / self.feature_extraction_method / "text/wavcaps_word_embeddings_vggsound_normed.npy").resolve(),
                    allow_pickle=True).item()
                ...
                data_list = [np.concatenate((data_raw_list[i], data_raw_wavcaps_list[i]), axis=0) for i in range(len(data_raw_list))]

                result['data'] = data_list

            else:
                result["data"] = list(data_raw_sorted.values())
            result["target"] = [self.class_to_idx[self.text_label_mapping[key].lower()] for key in list(data_raw_sorted.keys())]
...
class ContrastiveDataset(data.Dataset):
    def __init__(self, zsl_dataset):
        ...
        self.classes = self.zsl_dataset.classes # property of zsl_dataset returning indices of all classes
        if self.dataset_split == "train" or self.dataset_split == "train_val":
            ...
            # contains for each target the indices where the target occurs in the dataset
            self.target_to_indices = {target: np.where(self.zsl_dataset.targets == target)[0]
                                      for target in self.targets_set}
    ...
    def __getitem__(self, index):
        if self.dataset_split == "train" or self.dataset_split == "train_val":
            positive_target = self.targets[index].item() # get the target for specified index
            pos_target_index = list(self.targets_set).index(positive_target) # get the index of this target in the set of all targets (to get w2v embedding for this target)
            ...
            # for evaluation on validation set or test set we use pre calculated val_pairs to get the negative sample
...
class UCFDataset(data.Dataset):
    ...
    def map_embeddings_target(self):
        ...
        for i in range(len(sorted_classes)):
            mapping_dict[int(sorted_classes[i])] = i
        # mapping_dict returns a dictionary of form {class_idx: i}
        # with i from 0,..., n_classes
        return w2v_embedding, mapping_dict
    ...
    @property
    def targets(self):
        # mask that is True if target is in self.classes
        classes_mask = np.where(np.isin(self.data["audio"]["target"], self.classes))[0]
        return self.data["audio"]["target"][classes_mask] # return only those targets that are in self.classes
    ...
    @property
    def text_label_mapping(self):
        df = pd.read_csv(self.root / "class-split/ucf_clip_class_names.csv")
        return {val: df.original[idx] for idx, val in enumerate(df.clip_class_name)}
...
class ActivityNetDataset(data.Dataset):
    ...
    def get_data_by_modality(self, modality, dataset_type="train"):
        ...
        if modality == "text":
            data_raw = np.load(
                (
                        self.root / "features" / self.feature_extraction_method / "text/word_embeddings_activitynet_normed.npy").resolve(),
                allow_pickle=True).item()
            data_raw_sorted = dict(sorted(data_raw.items()))
            ...
            if self.args.use_wavcaps_embeddings == True:
                data_raw_wavcaps = np.load(
                    (
                            self.root / "features" / self.feature_extraction_method / "text/wavcaps_word_embeddings_activitynet_normed.npy").resolve(),
                    allow_pickle=True).item()
                data_raw_wavcaps_sorted = dict(sorted(data_raw_wavcaps.items()))
                data_raw_list = list(data_raw_sorted.values())
                data_raw_wavcaps_list = list(data_raw_wavcaps_sorted.values())
